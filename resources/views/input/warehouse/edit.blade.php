...
<div class="container m-0">
    <div class="row">
        <div class="col-6">
            <form class="shadow-sm rounded-2 px-3 py-2"
                action="{{ route('warehouse.update',$warehouse->id_warehouse) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control shadow-sm text-me  @error('address') is-invalid @enderror"
                        id="address" name="address" value="{{ $warehouse->address }}">
                    @error('address')
